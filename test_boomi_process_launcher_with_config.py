...
from boomi_process_launcher_with_config import (
    BoomiAPI,
    ScriptExitException,
)
import datetime
...
class TestPrintLogMessage(ut1.TestCase):
    ...
    def test_format_log_message(self, mock_datetime, mock_boomi_api):
        ...
        datetime_now = datetime.datetime.strptime("2024-01-01 08:01:02", "%Y-%m-%d %H:%M:%S")
        mock_boomi_api.now.return_value = mock_datetime.now.return_value = datetime_now
        groups = [
        ...
        for lines, group1, group2, group3, group4 in groups:
            with self.subTest(lines=lines, group1=group1, group2=group2, group3=group3, group4=group4):
                ...
                if lines == '1only':
                    self.assertEqual(f"{datetime_now}\t{group1}", log)
                elif lines == '2':
                    self.assertEqual(f"{datetime_now}\t{group1.ljust(self.boomi.GROUP1_LENGTH)}", log)
                elif lines == '1+2+n':
                    self.assertEqual(f"{datetime_now}\t{group1.ljust(self.boomi.GROUP1_LENGTH)}{group2}\n\t\t\t\t{group3}", log)
                elif lines == '4':
                    self.assertEqual(f"{datetime_now}\t{group1.ljust(self.boomi.GROUP1_LENGTH)}{group2}\n\t\t\t\t{group3}\n\t\t\t\t{group4}", log)
